...
Contribution pull requests contain only canonical record, registry and optional photo files whose paths are unique to that submission. They do not edit `data/measurements.csv`; the archive rebuilds that generated discovery index from canonical records after each merge.
